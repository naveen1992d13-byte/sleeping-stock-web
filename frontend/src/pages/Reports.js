...
function ReportDrawer({report,user,onClose}){
  ...
  const download=async()=>{if(!canExport)return toast.error('Excel download restricted to Admin / Master Admin');setBusy(true);try{const r=await axios.get(`${API}/reports-center/export/${report[0]}`,{params:f,responseType:'blob'});const url=URL.createObjectURL(r.data);const a=document.createElement('a');a.href=url;a.download=`NMTS_${report[2].replaceAll(' ','_')}_${f.from_date}_${f.to_date}.xlsx`;a.click();URL.revokeObjectURL(url);toast.success(`${report[2]} downloaded`);}catch(e){toast.error(e.response?.data?.detail||'Report generation failed');}finally{setBusy(false)}};
  return <div className="nmts-modal-overlay" role="presentation" onMouseDown={(e)=>{if(e.target===e.currentTarget)onClose();}}><div className="nmts-modal-panel max-w-lg" onMouseDown={(e)=>e.stopPropagation()}><div className="nmts-modal-header"><div><span className="text-emerald-600 font-bold text-sm">{report[1]}</span><h2 className="nmts-modal-title">{report[2]}</h2><p className="text-xs text-gray-500 mt-1">{report[3]}</p></div><button type="button" className="nmts-modal-close" onClick={onClose} aria-label="Close"><X className="h-5 w-5"/></button></div><div className="nmts-modal-body"><ScopeFilters user={user} value={f} onChange={setF} minDate={defaultFrom}/>{canExport ? <button disabled={busy} onClick={download} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60"><Download/>{busy?'Generating Excel...':'Excel Download'}</button> : <p className="mt-6 text-sm text-gray-500">Excel download is available to Admin and Master Admin only. You can still view report filters and on-screen data.</p>}</div></div></div>
}

function ReportsTab({user}) { const [selected,setSelected]=useState(null); return <><div className="nmts-report-card-grid">{reports.map(r=>{const Icon=r[4];return <button key={r[0]} type="button" onClick={()=>setSelected(r)} className="nmts-report-card"><div className="flex justify-between items-start"><span className="text-xl font-black text-gray-200">{r[1]}</span><div className="rounded-lg bg-emerald-50 p-2 text-emerald-600"><Icon size={18}/></div></div><h3>{r[2]}</h3><p>{r[3]}</p></button>})}</div>{selected&&<ReportDrawer report={selected} user={user} onClose={()=>setSelected(null)}/>}</> }
...
export function Reports(){const {user}=useAuth();const [tab,setTab]=useState('reports');return <div className="space-y-4" data-testid="reports-center"><div className="flex gap-2 rounded-xl border bg-white p-1.5 shadow-sm max-w-md">{[['reports','Reports'],['old','Old Reports']].map(([k,n])=><button key={k} type="button" onClick={()=>setTab(k)} className={`flex-1 rounded-lg px-3 py-2 text-sm font-semibold ${tab===k?'bg-emerald-600 text-white':'text-gray-600 hover:bg-gray-50'}`}>{n}</button>)}</div>{tab==='reports'?<ReportsTab user={user}/>:<OldReportsTab user={user}/>}</div>}
