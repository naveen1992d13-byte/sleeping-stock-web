...
export function Reports(){const {user}=useAuth();const [tab,setTab]=useState('reports');return <div className="space-y-6" data-testid="reports-center"><div className="nmts-module-header"><div className="flex items-center gap-3"><BarChart3 size={34} className="nmts-module-header-icon"/><div><h1>Reports Center</h1><p>Current-month reports, management analytics and secure historical report requests</p></div></div></div><div className="flex gap-2 rounded-2xl border bg-white p-2 shadow-sm">{[['reports','Reports'],['analytics','Analytics'],['old','Old Reports']].map(([k,n])=><button key={k} onClick={()=>setTab(k)} className={`flex-1 rounded-xl px-4 py-3 font-semibold ${tab===k?'bg-emerald-600 text-white':'text-gray-600 hover:bg-gray-50'}`}>{n}</button>)}</div>{tab==='reports'?<ReportsTab user={user}/>:tab==='analytics'?<AnalyticsTab user={user}/>:<OldReportsTab user={user}/>}</div>}
