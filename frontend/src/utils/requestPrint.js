...
export function buildRequestPrintHtml(group, logoUrl = '') {
  ...
  const pageHtml = pages.map((pageItems, pageIndex) => `
    <section class="page">
      ...
      <tbody>${pageItems.map((i, idx) => `<tr><td>${pageIndex * ITEMS_PER_PAGE + idx + 1}</td><td>${esc(i.part_number)}</td><td>${esc(i.description || '-')}</td><td>${esc(i.loc_at_request || i.loc || '-')}</td><td>${nfmt(i.requested_qty)}</td><td>${nfmt(i.accepted_qty ?? i.approved_qty ?? 0)}</td><td>${esc(i.purchase_aging_days_at_request ?? i.purchase_aging_at_request ?? '-')}</td><td>${esc(i.sales_aging_days_at_request ?? i.sales_aging_at_request ?? '-')}</td><td>${esc(displayRequestStatus(i.status))}<br/><small>${esc(i.approval_remarks || i.remarks || '')}</small></td></tr>`).join('')}</tbody></table>
      ${pageIndex === pages.length - 1 ? `<div class="signatures"><div>REQUESTED BY<span>${esc(group.requested_user_name || '-')}<br/>${esc(group.requester_mobile || group.requested_user_mobile || '-')}</span><small>Name + Mobile</small></div><div>ACCEPTED / PICKED BY<span>${esc(group.accepted_user_name || group.decided_user_name || '-')}<br/>${esc(group.accepted_user_mobile || group.decided_user_mobile || '-')}</span><small>Name + Mobile</small></div><div>DISPATCHED BY<span>${esc(group.dispatched_user_name || '-')}<br/>${esc(group.dispatched_user_mobile || '-')}</span><small>Name + Mobile</small></div><div>RECEIVED BY<span>${esc(group.received_user_name || group.completed_user_name || '-')}<br/>${esc(group.received_user_mobile || group.completed_user_mobile || '-')}</span><small>Name + Mobile</small></div></div><div class="notes"><b>Notes:</b> Please verify part number, accepted quantity and LOC before dispatch.</div>` : `<div class="continued">(Contd... Page ${pageIndex + 2})</div>`}
    </section>`).join('');
